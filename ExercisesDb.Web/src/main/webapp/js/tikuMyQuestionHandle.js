...
function InitNodeTree(json) {
    $("#treeContent").unbind().html("").jstree({
        ...
        "html_data": {
            //"data": json
            "ajax": {
                "url": "/zujuan/getMLOrZSDTreeInfo?isMLorZSD=" + isMLorZSD + "&collect=" + 1, //&num=" + json + " //不需要拼接 num 参数，data 代码会自动将 id 属性传入url 为参数
                "data": function (n) {
...
function printSTInfo() {
    ...
    if (jsonSTCount > 0) {
        $.each(jsonST, function (i, value) {
            ...
            //alert(value.st.客观题);
            if (value.st.objective && value.st.objective == "True") {
                //value.st.答案.substr(0, value.st.答案.Length - 1)
                var answerNew = value.st.answer;
                //alert(answerNew.substring(0, answerNew.length - 1));
            ...
            if (value.isFav == "是") {
                ...
                stContentHtml += "<a title=\"从我的题库移出试题\" class=\"buo yctkc\" id=\"collect" + value.st.tid1 + "\" href=\"javascript:;\">移出我的题库</a>";
            } else {
                // stContentHtml += "<a title=\"加入试题到我的题库\" class=\"buo jiatk\" id=\"collect" + value.st.tid1 + "\" href=\"javascript:;\">加入我的题库</a>";
                stContentHtml += "<a title=\"加入试题到我的题库\" class=\"buh jiatkc\" id=\"collect" + value.st.tid1 + "\" href=\"javascript:;\">加入我的题库</a>";
            }
            if (value.isInPaper == "是") {
